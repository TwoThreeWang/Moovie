网站名称 Moovie，是一个聚合电影搜索网站，（Golang + Gin + htmx + PG、无 Redis、JWT鉴权）

- 结构清晰，利于 SEO
- 页面职责单一，方便用 htmx 做局部刷新
- 登录态简单可靠，不依赖内存状态
- 全部状态可从数据库或 Cookie 推导
- 不要用 htmx 拼主内容，只用 htmx 做：收藏、历史、评分、反馈

原则：

- 可被搜索引擎索引的页面全部是 GET + 完整 HTML
- 私有页（dashboard、admin）必须鉴权

为了确保网站功能完备、用户体验良好且具备 SEO 优势，构建以下页面：

## 核心浏览流：
- 首页 (Index)：搜索入口、搜索历史、追剧历史。
- 搜索结果页 (Search)：聚合多源结果、高级筛选。
- 分类/发现页 (Discover)：按题材、地区、年代、评分筛选。
- 排行榜 (Rankings)：今日热搜榜、豆瓣/IMDb同步榜单。

## 内容呈现流：
- 电影详情页 (Detail)：剧情介绍、演职员表、多线路播放列表。
- 综合播放页 (Player)：带选集、线路切换、报错功能。
- m3u8 专用播放器 (HLS Player)：极简、无插件的底层播放页面。

## 用户系统：
- 登录/注册页 (Auth)：支持社交账号登录。
- 用户中心 (Dashboard)：收藏夹、观影历史、账号设置。

## 互动与支持：
- 热门搜索指标 (Trends)：可视化数据看板。
- 反馈建议/求片页 (Feedback)：用户提交缺失资源或Bug。
- 关于/声明页 (About/DMCA)：版权声明、免责协议、联系方式。

## 后台与 SEO：
- 后台管理系统 (Admin)：资源爬虫监控、用户管理、广告控制。
- 隐私政策/服务协议 (Legal)：合规性文档。
- Sitemap.xml：站点地图（自动化生成）。  
