...
func (r *HistoryRepository) Upsert(h *model.WatchHistory) error {
	return r.db.Clauses(clause.OnConflict{
		Columns:   []clause.Column{{Name: "user_id"}, {Name: "source"}, {Name: "vod_id"}},
		DoUpdates: clause.AssignmentColumns([]string{"douban_id", "title", "poster", "episode", "progress", "last_time", "duration", "watched_at"}),
	}).Create(h).Error
}
